...
namespace WordExcelExport
{
    public class ExcelExport
    {
        ...
        public const string T_DanhMucHD = "DanhMucHD";
        public const string T_DanhMucNhapHang = "DanhMucNhapHang";
        #endregion
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer, string tongthunhap)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%TongThuNhap", tongthunhap);
            }
        }
        private void BuildReplacerCurrentDateNhapHang(ref Dictionary<string, string> pReplacer, string tongchi)
        {
            if (pReplacer != null)
            {
                DateTime currentDate = DateTime.Now;
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
         ...
         //   string path = Global.AppPath + Constants.FOLDER_TEMPLATES + Constants.CHAR_FLASH; //+ Constants.FOLDER_EXCELS + Constants.CHAR_FLASH;
            ...
            switch (viewName)
            {
                ...
                case T_DanhMucHD:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("DanhMucHD.xls").ToArray();
                    break;
                case T_DanhMucNhapHang:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("DanhMucNhapHang.xls").ToArray();
                    break;
                    #endregion
            }
            // Get stream
        ...
        public bool ExportHD(List<HDBAN> dataSource, ref string fileName,string tongthunhap, bool isPrintPreview)
        {
            // Check if data is null
        ...
        //    replacer.Add(V_TONGSO, "100");
            ...
            BuildReplacerCurrentDate(ref replacer,tongthunhap);

            return OutSimpleReport(dataSource, replacer, "DanhMucHD", isPrintPreview, ref fileName);
        }
        public bool ExportNhapHang(List<NHAPHANG> dataSource, ref string fileName, string tongchi, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            BuildReplacerCurrentDateNhapHang(ref replacer, tongchi);

            return OutSimpleReport(dataSource, replacer, "DanhMucNhapHang", isPrintPreview, ref fileName);
        }
